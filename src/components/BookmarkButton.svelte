...
<span class="bookmark-control">
	<button class="tool-button" class:is-saved={saved} aria-pressed={saved} aria-label={`${saved ? "取消收藏" : "稍後閱讀"}：${title}`} disabled={!ready} on:click={() => { failed = !toggleBookmark(slug); }}>
	...
	{#if failed}<span class="storage-error" role="status">瀏覽器無法儲存收藏，請檢查儲存空間設定。</span>{/if}
</span>
