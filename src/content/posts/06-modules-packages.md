---
title: Python 模块与包：组织和复用程序
published: 2026-09-17
updated: 2026-09-17
description: 学习 import、模块名称空间、包结构、入口保护、相对导入和导入缓存，把程序拆分成可维护的文件。
tags: [Python, 基础篇]
category: Python 学习
draft: false
lang: zh_CN
...
前置知识：函数、作用域、列表与字典、文件和文件夹。类型提示在 第 5 章 已介绍；看到 `price: int` 时，记得它是提示，不是自动验证。

这章附有实际可执行的 module_demo 小项目。可以先执行一次，再按下文逐步拆解。所有例子都在本机运行，不需外部服务。

## 2. 先创建直觉

### 2.1 模块像一个有自己名字表的工具抽屉

对本章的自定义程序来说，一个 `.py` 文件通常就是一个模块，例如 `pricing.py`。模块也可能来自 Python 内建功能或其他加载方式，所以不要把所有模块都限定成普通文字文件。
...
Python 也支持不含 `__init__.py` 的 namespace package；「文件夹没有 `__init__.py` 就永远不能导入」并不准确。初学时先使用一般包，目录关系最清楚。

### 2.3 导入是加载与绑定，不是复制全部程序代码

第一次用某个模块名称导入时，Python 会查找、创建模块对象并执行它的顶层程序代码。这些「顶层程序代码」包括函数定义、变量赋值，也包括没有放在函数内的 `print()` 或 `input()`。

同一个 Python 进程内，再用相同完整名称导入，通常会从 `sys.modules` 缓存取得已有模块。模块已完成的顶层程序代码一般不会再执行一次。
...
`get_product()` 返回商品字典；找不到时，`dict.get()` 默认返回 `None`。它不负责打印商品，也不要求用户输入。

本例直接返回原字典的参考，外部若修改内容，也会改到模块保存的数据；它不是自动复制的安全副本。这里只读取，后续要让其他文件修改数据时，应把修改入口和规则设计清楚。
...
`line_total()` 返回整数分，`format_money()` 返回格式化字符串，两个函数本身都不显示输出。这份格式化函数以非负金额为前提，不是通用的金融格式化工具。类型提示也不会阻止负数、字符串等不符合预期的输入，互动程序需要在调用前验证。
...
`__all__` 主要用来说明 `from module_demo import *` 要带出的名字；它不是存取权限或安全限制，也不表示所有子模块都自动加载。即使没有把 `catalog` 写进这个清单，仍可明确导入 `module_demo.catalog`。
...
再开一个新 Python 进程，从 `examples` 执行以下片段。可以把片段存成 `examples/check_entry.py`：
...
# True
# True
# 说明：相同完整模块名在同一进程中复用模块；del 只删掉当前的一个名字。
import sys
...
缓存是**每个 Python 进程自己的**，不是把程序执行结果永久存到磁碟。重新启动程序，顶层程序代码会重新执行。`__pycache__` 下的 `.pyc` 是字节码缓存，和 `sys.modules` 中当前这次执行的模块对象不是同一回事。

缓存也以模块名称为键。若把同一来源文件当 `__main__` 执行，又用一般模块名导入，可能得到两次执行与不同状态。初学时不要用修改 `sys.modules` 或反复 reload 的方式处理设计问题；先维持清楚、一致的入口。
...
```python
# 输出：无。
# 说明：用来观察模块名称、全局查找及 from 导入后的绑定关系。
PRICE = 10
TAGS = ["new"]
...
# 999
# 10
# 说明：get_price() 的全局名称查找使用它定义时所在的模块。
...
仍使用 `binding_demo.py`，从 `examples` 的**新进程**执行：
...
一般情况下，Python 先查看已加载模块的缓存；若需要加载，再透过导入机制查找。对本章的文件型模块，`sys.path` 是重要的搜索位置清单，但内建模块或特殊加载器不全都靠普通文件路径。

常见的 `sys.path` 来源包括：

- 脚本所在文件夹；使用 `python -m ...` 或互动模式时，通常包含当前工作文件夹。
- `PYTHONPATH` 环境变量提供的位置（如果有设置）。
- Python 标准库的位置。
- 当前 Python 环境安装第三方包的位置，例如 `site-packages`。

具体顺序和内容可能受虚拟环境、启动选项或环境设置影响。最常见的初学错误，是把「当前终端文件夹」「正在执行的文件所在文件夹」「项目顶层」当成永远相同的位置。

从 `examples` 执行以下片段，检查实际加载的文件是否来自预期示例。这里的 `__file__` 是文件型模块的来源路径；不保证所有种类的模块都有它。
...
`sys.path` 是可修改列表，`sys.path.insert(0, 某个文件夹的字符串路径)` 能让当前进程优先查找该位置。若要找到 `module_demo`，应加入**包含 module_demo 的上一层文件夹**，而非只加入包内某个子模块所在位置。它不会替安装包，也不会永久改变其他进程。

对这份小项目，从正确的 `examples` 目录以 `-m` 启动即可。不要把自己电脑的绝对路径散落到每个文件；搬到另一台电脑就可能失效，加入过度优先的路径也可能导入错误的同名模块。

常见例子是把自己的文件取名为 `json.py`、`random.py` 或 `math.py`，与标准库名称冲突；程序可能加载自己的文件。报错时可检查实际来源。也要避免创建 `module_demo.py` 与 `module_demo/` 同时存在于搜索位置，让自己难以判断要加载哪个。
...
- 共用设置可放 `settings.py`，设置模块不应反过来启动入口。
...
| 2 | 首次加载包，执行 `__init__.py` | 无，只定义名字 |
| 3 | 把 `module_demo/__main__.py` 作为入口执行 | 尚无 |
| 4 | 相对导入 `catalog` 和 `pricing` | 无，只创建数据和函数 |
| 5 | 定义 `main()`，判断入口条件成立 | 尚无 |
| 6 | 调用 `main()`，取出 `P001` | 无，取得字典 |
| 7 | `line_total(500, 3)` 返回 `1500` | 无，得到整数分 |
| 8 | 前两次 `print()` | 显示商品、数量 |
| 9 | 第三次 `print()` 的参数先求值，其中 `format_money(1500)` 返回 `"15.00"` | 取得字符串，尚未显示小计 |
| 10 | 第三次 `print()` 使用组合好的字符串 | 显示小计 |

函数定义会创建函数，但不等于调用函数。这个区分正好接上前章的 `return`：计算结果先返回，直到外层选择 `print()` 才显示。
...
| 改了文件，旧互动工作阶段没变 | 模块是否已被缓存 | 初学先重新启动程序再验证，不把重复 import 当成重载 |
...
| 写了 `__all__` 仍可导入其他名字 | 把导出清单误当成权限控制 | `__all__` 不是禁止存取的安全机制 |

读错误时先看最后一行的种类与讯息，再往上找自己写的文件。不要看到「找不到」就立刻安装包；自己写的模块根本不需要去网路安装。异常处理与测试补充见 bridge-errors-testing.md。
...
- `__init__.py` 保持简短。只有需要方便用户导入时，才考虑清楚地重新导出工具。
- 不用 `from ... import *` 隐藏来源，也不要把内建或常用标准库名称拿来当自己的文件名。

未来项目变大时，可以再分出 `tests/`、`data/` 和项目说明文件。包的顶层应清楚可找到；不要为了看起来专业就应用自己还无法解释的多层目录。
...
| 自查问题 | 简答 |
|---|---|
| `import pricing` 是把所有函数复制进当前文件吗？ | 不是。通常加载或取得模块对象，并绑定 `pricing` 这个名字。 |
| 模块的顶层 `print()` 什么时候执行？ | 首次加载时；同一进程相同名字的重复导入通常用缓存。作为脚本重新启动又是新的一次执行。 |
| 导入函数后，它的全局变量在哪里找？ | 在定义该函数的模块全局名称空间，而非任意调用者的同名变量。 |
...
| 删除当前 `module` 名字，会立即卸载模块吗？ | 不会。缓存与其他对象仍可能持有参考。 |
| `sys.modules` 和 `__pycache__` 一样吗？ | 不一样。前者是当前进程内的模块缓存；后者通常保存编译后的字节码文件。 |
| 循环导入一定报错吗？ | 不一定；问题常是模块尚未初始化完成，就被获取尚未定义的名字。应优先整理依赖方向。 |
| `-m` 后面写 `module_demo.py` 吗？ | 不写。应是可导入的模块／包名称，例如 `module_demo`。 |

本章完成的最低标准：能从正确位置启动附带项目、说清楚四个主要文件的分工、理解导入缓存与名称绑定，并能把自己的小程序拆成数据、计算和入口。后续增加功能时，先维持这种可解释的结构，再逐步扩大。
