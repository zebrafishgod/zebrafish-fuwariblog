---
title: Python 哈希、正则与日志：验证数据和排查问题
published: 2026-09-18
updated: 2026-09-20
description: 学习哈希摘要、密码安全基础、正则表达式和 logging，用于校验内容、验证文本格式并记录程序运行过程。
tags: [Python, 基础篇]
category: Python 学习
draft: false
lang: zh_CN
...
# 第 8 章｜哈希、正则与日志：检查数据，理解程序发生了什么

本章对应基础篇目录约第 340～363 集，主题包括内建 `hash`、`hashlib`、正则表达式，以及 logging 的设置、logger、handler、命名与日志分类。视频中穿插的包补充，集中整理在 第 6 章：模块与包。本章依已核对的目录整理，不是逐集视频的逐字稿。

前置知识：字符串、字节的基本概念、字典、函数、文件与模块。本章需要处理少量异常；相关语法可配合 错误处理与验证桥接 阅读。

本章回答三个不同问题：

| 工具 | 它帮回答什么？ | 例子 |
|---|---|---|
| 哈希摘要 | 这份内容和之前是否一致？ | 备份文件与原文件是否相同 |
| 正则表达式 | 这段文字是否符合某种形式？ | 商品 ID 是否为 P 加三个数字 |
| 日志 | 程序何时做过什么？哪一步失败？ | 订单创建成功、数据加载失败 |

学会这些工具后，程序会更容易验证与排错。它们仍然要创建在正确的数据结构与业务流程上。

---

## 8.1 内建 hash：先理解字典为什么能按 key 查值

可以把哈希理解为：根据一份数据，算出一个便于比较和查找的数值。

Python 的字典和集合会使用哈希机制协助查找。内建 `hash()` 返回整数，但它主要服务 Python 执行期间的数据操作。
...
# 在同一进程中，相等且可哈希的对象必须有相同的 hash。
```

但是，**hash 相同不代表数据必定相同**。不同数据有可能得到同一个哈希值，这叫做碰撞。字典不会只靠哈希数字决定两个 key 相等，还会进一步处理比较。

常见的可哈希数据包括字符串、整数，以及所有成员都可哈希的元组。列表、字典、集合通常不可哈希，所以不能直接作字典 key。

注意元组本身不可变，不代表它里面的成员都可哈希：`(1, 2)` 可以，包含列表的 `(1, [2])` 就不可以。

### 为什么不能用 hash() 当永久文件指纹？

Python 默认会对字符串、字节等的哈希加入进程相关的随机化。同一段文字，在下次重新启动 Python 后，`hash()` 的数字可能不同；不同环境也不应假设完全一致。
...
## 8.3 SHA-256 文件校验：一块一块读，避免一次加载整份大文件
...
这是理解数据流的独立示例，不是购物系统作业的必要前提，也不要求在基础篇实现登录系统。
...
# make_password_record() 返回设置与摘要数据，不返回或保存明文密码。
# compare_digest() 返回比较结果，适合用于这类敏感摘要比较。
```

此处 `600_000` 是示范成本参数，不是适用所有装置和所有服务的永久设置。实际系统应采用成熟认证框架、依当时建议与性能测试选择参数，并验证保存纪录的格式和算法版本。

此刻需要记住的是：SHA-256 文件校验、Python 的 `hash()` 和密码保存，虽然都提到「哈希」，却有不同要求。
...
## 8.6 r 字符串：先分清 Python 的转义与正则的转义

同一个反斜线可能先被 Python 字符串语法解读，再交给正则引擎。为了少处理一层转义，正则通常使用 `r"..."` 原始字符串。
...
# 说明：
# "\n" 是一个换行字符；r"\n" 保存反斜线和 n 两个字符。
# 正则里的 \. 表示字面上的小数点；未转义的 . 有特殊意义。
```

`r` 只改变 Python 对字符串的处理方式，**不会关闭正则语法**。例如 `r"\d"` 仍会被正则引擎解读为数字类别。

另一个常见错误是用 `"\bword\b"` 想写单词边界，但 Python 会先把 `\b` 当成退格字符。此时应使用 `r"\bword\b"`。
...
| `$` | 字符串末尾或末尾换行之前；受 MULTILINE 影响 | `end$` |
| `A\|B` | A 或 B 其中一种规则 | `red\|blue` |
| `(...)` | 分组并捕获内容 | `(P[0-9]{3})` |
| `(?:...)` | 分组但不捕获 | `(?:cat\|dog)` |
| `\b` | 单词边界 | 边界依 `\w` 与非 `\w` 判断 |
...
「多行」和「点号包含换行」是不同设置，不要把 `re.M` 与 `re.S` 当成同一件事。
...
# False
# 说明：
# text 是全形数字；默认 \d 接受这些 Unicode 十进位数字。
...
| `re.findall` | 找到全部不重叠匹配 | 列表；元素受捕获分组影响 |
...
# findall() 找到的是字符串，若要相加仍需做 int() 等类型转换。
```

做商品 ID 等完整输入验证时，优先考虑 `fullmatch`。只用 `match`，可能会让 `"123abc"` 这种末尾多了文字的输入通过原本只想允许数字的检查。
...
# [('P001', '2'), ('P002', '10')]
# 说明：
# 无捕获分组：返回完整匹配；一组：返回该组字符串；多组：返回元组。
# 只是想把规则括成一组而不改变捕获结果，可以使用 (?:...)。
```

不要只看正则能不能匹配，也要确认它返回的数据形状。这和学习列表、元组、字典时的思路相同。

## 8.10 贪心匹配与非贪心匹配：同样能匹配时，要取多长？

`*`、`+`、`{m,n}` 等重复规则默认倾向取较长的匹配，但仍要让整个规则成功，这常称为贪心匹配。在量词后加 `?`，则先尝试较短的匹配。
...
「非贪心」不是「最多一个字符」。它仍可能匹配很长的内容，只要完成整个规则需要那么长。

正则适合有明确文字规则的任务。遇到完整 JSON、嵌套结构、HTML 文件等，优先使用对应解析器，避免把复杂数据格式硬塞进一条正则。
...
同一个事件可以在画面上简短提示，在日志里保留更有用的技术信息。
...
# WARNING: 铅笔库存偏低
# 说明：
# DEBUG 低于设置门槛，不会显示；INFO、WARNING 会显示。
# 日志函数执行记录动作并返回 None，不是返回格式化后的字符串。
```

这里明确使用 `sys.stdout`，让例子的显示顺序容易核对。默认串流 handler 通常使用 `stderr`，所以有时会在编辑器的错误输出区看见一般日志；这不代表每一行都是错误。

`basicConfig` 一般只在根 logger 尚无 handler 时生效。本章的独立示例使用 `force=True`，使重跑示例时能重新设置；正式应用通常在程序入口集中配置一次，模块内只取得 logger 并使用。

## 8.13 Logger、Handler、Formatter：谁说话，送去哪，长什么样
...
本节使用库提供的工具对象，不要求会自定义类别。先理解讯息如何通过这条路径：

**日志调用 → logger 的有效级别 → handler 的级别 → formatter → 输出位置。**

如果 logger 已经挡掉 DEBUG，即使文件 handler 设成 DEBUG，也拿不到那条纪录。

`logger.info("订单 %s 已创建", order_id)` 会让 logging 处理参数格式化。`%s` 是日志文字的占位符，与正则语法无关。不要把密码、登录凭据或完整个人数据直接放进日志。
...
在模块里通常使用 `logging.getLogger(__name__)`，让 logger 名称与模块名称一致。例如汇入 `shop.checkout` 模块时，`__name__` 就能反映模块来源；直接执行入口文件时通常是 `"__main__"`。模块与包的名称关系见 第 6 章。
...
另外，重复执行设置函数、每次都 `addHandler` 却没有移除旧 handler，也会让日志重复。
...
当设置包含多种格式、多个 handler、不同模块 logger 时，一连串 `setLevel`、`addHandler` 容易分散。`logging.config.dictConfig` 可以把配置整理在一个字典中。
...
# 说明：
# shop.checkout 没有自行设置级别，因此从父 logger shop 取得 INFO 门槛。
# 纪录传给 shop 的 console handler；shop 的 propagate=False 阻止继续向上。
# dictConfig() 应用设置并返回 None。
```
...
3. `loggers["shop"]` 引用 `"console"`，并设置传播方式。
...
日志和错误处理是两件工作：日志留下信息；程序仍要决定重试、返回失败结果、提示用户，或重新抛出异常。

不要因为写了一行日志，就让后续程序假装失败的操作已经成功。异常处理与测试的完整桥接，见 错误处理与验证桥接。
...
`maxBytes` 不是绝对不会超过的硬上限：如果单条消息本身就非常大，文件也可能超出设置。因此日志内容本身也应合理控制。
...
6. 成功时用 INFO 记录结果；发生文件或解析异常时记录必要信息。
7. 若需要核对备份内容，再用 SHA-256 计算文件摘要。

这条流程里，正则不负责查库存，哈希不负责解析 JSON，日志也不负责回滚交易。
...
| 为什么不保存 `hash("P001")` 当永久校验值？ | 字符串 hash 跨进程可能改变，这不是它的用途。 |
...
| `r` 会让正则符号变成普通字符吗？ | 不会，它主要控制 Python 字符串的转义处理。 |
| `\d` 默认等于 `[0-9]` 吗？ | 不等于，它也能接受其他 Unicode 十进位数字。 |
...
| `logger.exception` 除了文字还会留下什么？ | 正在处理的异常 traceback。 |
| 同一日志印两次，先检查什么？ | 重复添加 handler，以及子父 logger 同时输出且仍向上传播。 |
