---
title: Python 数据处理：字符串、容器与文件读写
published: 2026-09-10
updated: 2026-09-13
description: 学习字符串处理、列表与字典等容器、引用与复制、字符编码，以及文本和二进制文件的读写。
tags: [Python, 基础篇]
category: Python 学习
draft: false
lang: zh_CN
...
前置知识：变量、基本数据类型、`print()`、`input()`、`if`、`for`、`while`。本章使用 Python 3；第 168 集的函数引入放在下一章。

每个 Python 程序代码框都是一个可单独执行的例子。先预测结果，再执行，最后只改一处重跑。文件例子会创建自己的暂存文件夹，结束后自动移除；例子的 `w` 模式只操作示例文件。
...
追踪第一个转换：`number` 指向整数 10，`bin()` 取得这个值，返回字符串 `"0b1010"`，`print()` 显示字符串。不是将整数改成另一种类型的「二进制整数」。
...
`int(text, base)` 中，`text` 是交给函数处理的文字，`base` 指定它使用的进制，转换结果称为返回值。这只是使用现成函数，自定义函数下一章才开始。

常见错误：`int("102", 2)` 会失败，因为二进制没有数字 2；`int("10")` 默认按十进制解读。
...
把 `"Python"` 看成六个依次排列的字符。正索引从 0 起算；`-1` 是最后一个。切片格式为 `text[start:stop:step]`，包含起点，不包含终点。
...
索引超界会出错；切片超过末尾通常只取到现有数据。空字符串可以切片，不能取第 0 个字符。步长不能是 0。
...
# 说明：无参数 split 合并连续空白；指定逗号拆分时保留空字段。
```
...
# 输出：笔记本 × 2 = 25.00 元。
# 说明：:.2f 控制显示两位小数，并不将浮点数变成精确的金额类型。
```

**练习：**把 `"  Alice, 85  "` 变成姓名 `"Alice"` 和整数 85；取出 `"order-2026-001"` 的最后三个字符。

**自查简答：**清理整段后用逗号拆分，再清理各字段并 `int()`；最后三个字符用 `text[-3:]`。方法的返回值要使用或保存。
...
「元组不可变」不代表里面所有对象都不可变。`record[1] = []` 不合法；`record[1].append(100)` 改的是内层列表，所以可以。元组适合座标、固定字段的纪录、函数的多个结果。
...
`key in product` 检查键。直接迭代字典也是取得键；`keys()`、`values()`、`items()` 分别提供键、值、键值对。`get()` 找不到时只返回默认值，不会新增键。`pop(key)` 删除并返回值；不确定存在时可用 `pop(key, None)`。

键必须可哈希；常用字符串或整数。元组只有在所有元素都可哈希时才可当键，包含列表的元组不行。「不可变」和「可哈希」不能当成完全相同的定义。
...
print(products["P002"])
# 输出：{'name': '铅笔', 'stock': 18}。
# 说明：先用商品 ID 取得一笔商品，再用 stock 取得字段。
```
...
`{}` 是空字典，空集合用 `set()`。集合元素也必须可哈希。`remove()` 删不存在的元素会报错，`discard()` 不会。`list(set(data))` 能去重，但不能保证保留原顺序。
...
print(queue.pop(0))
# 输出：B；A。
# 说明：堆叠后进先出，队列先进先出；列表 pop(0) 需移动后面的项目，只适合小型示范。
```

大量队列操作通常用 `collections.deque`。现在先理解规则，不必立即引入新结构。
...
# 说明：b = a 不复制列表；append 改共享对象，b = [9] 只重新绑定 b。
```
...
# 说明：浅拷贝只建新外层；深拷贝让本例的嵌套列表也分开。
```

`shallow.append()` 改新外层，`a` 没多一项；`shallow[0].append()` 改共享内层，`a[0]` 也变了。这里讨论一般嵌套数据，`deepcopy()` 并不是所有文件资源或外部连线都能复制的工具。
...
整数 `count += 1` 可先理解为算出新值再绑定，但不能把这个模型套到所有类型。

**练习：**用一句话说明 `a.copy()` 为什么不能隔离内层列表的修改。

**自查简答：**只复制外层容器，内层项目仍指向原对象。除错先分清「重新绑定名称」与「修改共享对象」。
...
Python 2 已停止支持；视频的 `raw_input()`、Python 2 字符串行为作历史理解。Python 3 使用 `input()`。多版本共存时，确认编辑器选的解释器与安装包用的是同一个环境。

### 6.1 源代码文件头和外部文件编码不是同一件事

Python 3 源代码默认按 UTF-8 解读，通常不必写编码声明。旧教材中的 `# coding: utf-8` 是在指定**这个 Python 源代码文件**的编码，不会替 `open()` 读写的所有外部文件设置编码。
...
print("源代码中的中文")
# 输出：源代码中的中文。
# 说明：编码声明示范放在前两行；shebang 在支持它的启动环境用来选择解释器，对 Python 语法而言是注解。
```

shebang 常见于 Unix 类系统可直接执行的脚本；Windows 的 Python launcher 也可能解读它，但不应假定任何编辑器或启动方式都据此选版本。执行 `python 文件名.py` 时，先由选的 `python` 决定执行环境。Python 3 仍可能因外部数据编码或终端显示设置不符而乱码，不能把「Python 3」当作永不乱码的保证。
...
`TemporaryDirectory()` 提供暂存文件夹；`Path(folder) / "note.txt"` 组成路径。它们是让示例可安全重跑的准备工具，当前先理解用途即可。
...
`t` 是文字模式（默认），`b` 是二进制模式，例如 `rb`、`wb`。`+` 是同时读写，追加要看 `a`。文字模式建议明写 `encoding="utf-8"`，二进制模式不能指定 `encoding`。
...
with TemporaryDirectory() as folder:
    ...
    with open(path, "r", encoding="utf-8", newline="") as file:
        print(repr(file.read()))
# 输出：'A\nB\nC\n'；'A\r\nB\rC\n'。
# 说明：默认 newline=None 读取时统一换行；newline="" 保留读到的换行形式。
```

文字写入时，`newline=None` 可能把换行转成平台形式；二进制模式则原样处理字节。一般文字先用默认，只有需求明确时才手动控制 `newline`。
...
为降低 I/O 次数，Python 可能先把写入数据放在缓冲区；操作系统也可能保留自己的缓存。机械硬盘有寻道等成本，固态硬盘没有同样的机械寻道，但也仍有 I/O 延迟。批量写入、逐行处理与分段搬运，是在延迟、内存用量与程序简单度之间取平衡。
...
先约定格式：每行 `账号:练习密码`，按第一个冒号分隔。账号不能含冒号或换行，密码不能含换行。真实账号不能明文保存密码，这里只练解析和写文件。
...
with TemporaryDirectory() as folder:
    ...
    print("登录成功：", users.get("alice") == "demo123")
    ...
    if new_name in users:
        ...
        print("注册完成：", new_name)
# 输出：{'alice': 'demo123', 'bob': 'abc:123'}；无效行数： 1；登录成功： True；注册完成： carol。
# 说明：略过空行，拒绝格式错误与重复账号；split(":", 1) 保留密码中的冒号。
```

追踪 `"bob:abc:123"`：最多拆一次，得到 `"bob"` 和 `"abc:123"`，再存进字典。此例使用确定符合规则的新增数据；改成 `input()` 时，必须先验证账号和密码格式。尚未处理多人同时写文件。
...
这个版本不处理标点，也不是中文分词器。扩充前先定义「单词」的规则，避免把不清楚的需求藏程序。
...
| `TypeError` | 操作与类型 | 留意 `str`/`bytes`、不可变字符串、可哈希限制 |
...
- 为何拆分后还要清理字段？整段 `strip()` 只处理最外侧，不处理每个逗号旁的空白。
