...
Python 2 已停止支持；影片的 `raw_input()`、Python 2 字符串行为作历史理解。Python 3 使用 `input()`。多版本共存时，确认编辑器选的解释器与安装包用的是同一个环境。

### 6.1 源代码文件头和外部文件编码不是同一件事

Python 3 源代码默认按 UTF-8 解读，通常不必写编码宣告。旧教材中的 `# coding: utf-8` 是在指定**这个 Python 源代码文件**的编码，不会替 `open()` 读写的所有外部文件设定编码。
...
print("源代码中的中文")
# 输出：源代码中的中文。
# 说明：编码宣告示范放在前两行；shebang 在支持它的启动环境用来选择解释器，对 Python 语法而言是注解。
```
