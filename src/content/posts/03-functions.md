...
前置知识：判断、循环、列表、字典、字符串拆分。学习路线是：定义与调用 → 返回值 → 参数系结 → 预设值 → 收集与解包 → 拆分一个完整流程。先不学装饰器。
...
定义括号里的 `name` 叫**形参**，是等待数据的名称；调用括号里的 `"小明"` 叫**实参**，是这次提供的数据。形参不是「所有调用共用一个全域变量」，每次调用都有自己的局部系结。
...
# 输出：三次都是 小明 今年 18 岁。
# 说明：位置实参依顺序系结，关键字实参依名称系结；这些形参都允许两种方式。
```
...
# 说明：第一个位置已系结 name，不能再给同名关键字；必要参数也不能漏掉。
```
...
对一般可按位置传入的形参，没有预设值的放前，有预设值的放后。此规则不应简化成「任何必要参数都只能在所有预设参数前面」；仅关键字参数有不同规则，下一小节补充。
...
**自查简答：**形参指函数定义中的名称；位置／关键字描述调用时如何给值。不能把「有预设值」误当成「只能用关键字」。
...
**练习：**把预设字典 `options={}` 改成每次独立创建。**自查简答：**预设设为 `None`，函数内 `if options is None: options = {}`。

## 5. 传入列表后，函数能不能改到外面？

Python 调用函数时，形参会系结到实参所代表的对象。不是把整个列表复制一份，也不是一律「只能传值／只能传址」的简单二分。
...
# 输出：[80, 90]；[80, 90] [100]。
# 说明：append 修改共同列表；scores = [100] 只重新系结函数内的名称。
```
...
| `TypeError` 缺少参数 | 调用不符合形参 | 依位置和名称画系结表 |
| 重复提供同一参数 | 位置已提供，又写关键字 | 移除重复 |
| 第二次混入第一次数据 | 修改了可变预设对象 | `None` 作预设，调用内新建 |
...
1. 写 `calculate_discount(price_cents, rate=0.9)`，返回折后金额。先约定小数如何处理；不要在函数内打印。这只是参数与返回值练习，完整电商作业会使用整数分作金额单位。
2. 写 `find_max_min(*numbers)`；没有数字时返回 `None`，否则返回最大值和最小值。
3. 写 `build_student(name, score, **extra)`，返回字典，并说明新增栏位如何处理。
4. 将上一章学生管理草稿拆成「查询学生」「新增学生」「计算平均分」「显示选单」等函数。每个函数写一句输入、返回值与副作用说明。
5. 将猜数字游戏拆成「判断本次猜测」「进行一局」「显示结果」。判断函数接收答案和猜测，返回偏大、偏小或正确，不自行读取输入。
...
- 预设值何时创建？执行函数定义时，不是每次调用时。
- 50 行函数一定不好吗？不能只看行数；看责任、数据流和分支是否清楚。
- 怎样才算能进下一章？能不看范例写函数，追踪参数如何对应，清楚区分返回值与副作用。
