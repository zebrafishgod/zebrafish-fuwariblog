---
title: Python 迭代器与生成器：从逐项处理到常用算法
published: 2026-09-16
updated: 2026-09-16
description: 理解可迭代对象和迭代器，掌握生成器、推导式、递归、二分搜索以及常见排序和筛选方法。
tags: [Python, 基础篇]
category: Python 学习
draft: false
lang: zh_CN
...
> 对应既有学习路线的 242–276 节范围，按概念重新编排。本章是依课程主题整理的自学讲义，并非逐集视频逐字稿。示例以 Python 3 为准。
...
3. 怎样用推导式、递归、二分搜索，把问题写得更清楚？
4. `lambda` 究竟是什么，为什么常和 `sorted()` 一起出现？

前置知识是列表 `list`、字典 `dict`、集合 `set`、条件、循环，以及函数的参数和 `return`。本章不需要先学类别。

读到捕捉 `StopIteration` 的例子时，先把 `try/except` 理解成「尝试取值；如果已经取完，就走另一段处理」。异常的完整说明见 补充：错误与测试。

建议分成三次学习：先掌握迭代与生成器，再学推导式与递归，最后做搜索与排序。`send()` 是选读，不是写出第一个完整程序的必要条件。
...
普通函数用 `return` 返回结果，这一次调用就结束。含 `yield` 的函数是**生成器函数**：调用它会先创建生成器对象；之后每次获取下一项，才从上次暂停处继续执行。

| 动作 | 做了什么 | 是否一定显示在萤幕？ |
|---|---|---|
| `print(value)` | 把内容显示到输出 | 是，默认显示在终端 |
| `return value` | 把结果返回函数调用处，结束这次调用 | 否 |
...
递归把问题拆成「同一种、更小的问题」；二分搜索在**已排序**的数据中，每次排除约一半候选。两者都不是「看起来比较高级就使用」，而是要看问题是否符合条件。
...
若「取不到」是正常情况，可以为 `next()` 提供默认值：

```python
# 输出：
# 10
# 没有下一项
# 说明：next(迭代器, 默认值) 在耗尽时返回默认值。
cursor = iter([10])
print(next(cursor, "没有下一项"))
print(next(cursor, "没有下一项"))
```

注意：`next([10, 20])` 不成立，因为列表可迭代，却不是迭代器。先写 `cursor = iter([10, 20])`。若数据本身可能就是默认值，便不能只凭「结果等于默认值」判断是否耗尽。
...
# 甲
# 乙
# 说明：这是 for 的核心取值流程；只在 next() 周围捕捉耗尽信号。
names = ["甲", "乙"]
cursor = iter(names)
...
重点是输出顺序：`stream = square_numbers(3)` 只创建对象，**不立即执行函数主体**。第一次 `next(stream)` 才开始执行；遇到 `yield 0` 暂停，把 `0` 返回给外层的 `print()`。
...
| 形式 | 结果 | 典型用途 |
|---|---|---|
| `[表达式 for 变量 in 数据]` | 列表 | 保存转换后的顺序数据 |
| `{键: 值 for 变量 in 数据}` | 字典 | 创建查找关系 |
| `{表达式 for 变量 in 数据}` | 集合 | 去除重复 |
| `(表达式 for 变量 in 数据)` | 生成器 | 之后逐项产生结果 |
...
先单独认识条件表达式，也常被称为「三元表达式」：`符合时的值 if 条件 else 不符合时的值`。它会计算出一个值，可放在赋值右侧；只有被选中的那一侧会求值。

```python
# 输出：
# 可购买
# 说明：条件表达式返回其中一个字符串；print() 才显示它。
stock = 3
...
# 输出：
# ['奇数', '偶数', '奇数', '偶数']
# 说明：if/else 是选值；末尾没有筛选条件，因此输出仍有四项。
...
需要随机取得第 100 项、反复遍历、确认长度时，列表通常更直接。只需扫过一次、数据很大时，再考虑生成器。生成器表达式的**结果元素**逐项计算，但最左侧 `for` 的可迭代表达式会在创建时求值；不要把「惰性」理解成所有东西都完全延后。
...
这个例子是 `3 + 3 + 1`。应用到先前学过的文件处理时，在 `with open(..., encoding="utf-8") as file:` 区块内，把 `lines` 换成 `file`，就能逐行读取并加总，避免先用 `readlines()` 创建完整行列表。必须在文件仍开启时完成消耗。这是「按空白分隔」的单词数，不是中文字数，也不是自然语言的精确分词；`len(text)` 计算的又是另一种长度，先定义需求再选算法。

### 3.6 选读：`send()` 不只取值，也能把值送回去

`next()` 只要求「继续执行」。`send(value)` 还把一个值送进当前暂停的 `yield` 表达式。
...
# 24
# 1
# 说明：factorial() 用 return 返回数字；外层 print() 才显示结果。
# 前提：n 是非负整数。
def factorial(n):
    if n == 0:
        return 1
...
不能只检查「有没有写 `if`」。如果输入 `-1`，每次再减一会离 `0` 越来越远；最终通常会遇到 `RecursionError`。本例把非负整数列为调用前提。要接收用户输入，还需要先检查范围与类型。

每次递归调用都要保留尚未完成的工作。Python 有递归深度限制；递归也通常有额外开销。像阶乘这种直线型累积，使用循环同样清楚；本例主要用来练习「拆解、到底、返回」。不要用调高限制代替修正无限递归。
...
# CAB
# CBA
# 说明：permutations() 返回排列列表；外层 for 和 print() 显示各排列。
...
基例为什么是 `[[]]` 而不是 `[]`？因为「没有剩余元素」仍有**一种**排列方式：空排列。外层才能把最后选中的元素接上去。若直接返回 `[]`，上层 `for rest in ...` 一次也不执行，全部结果就消失了。
...
`lambda 参数: 表达式` 创建函数，调用后返回表达式的值。它只有一个表达式，不能写多行叙述。较长逻辑就用 `def`，并取一个好懂的名字。

```python
# 输出：
# 7
# 7
# 说明：两个函数都返回加法结果；lambda 不需要再写 return。
...
# ('书', 30)
# [('书', 30), ('笔', 5), ('杯子', 20)]
# 说明：key 函数返回比较依据；sorted() 产生新列表，原列表顺序不变。
...
# [20, 30]
# []
# 说明：Python 3 的 map/filter 返回迭代器；list() 取得并保存结果。
prices = [10, 20, 30]
...
类型提示（type hints）把函数希望收到和返回的数据类型写清楚：`name: str` 表示预期字符串，`-> int` 表示预期返回整数。编辑器和额外的静态检查工具可用它提醒潜在问题；Python 执进程序时不会只因为写了注释，就自动转型或拒绝不符合的值。

```python
# 输出：
# 7
# 34
# 说明：第二次调用故意违反注释，展示类型提示本身不阻止字符串相加。
...
第二行的 `34` 是两个字符串连接后的结果，与整数 `34` 显示相似，但类型不同。这次调用应由类型检查工具提醒，业务程序不应故意这样使用。若数据来自 `input()`，仍然要自己转型、检查内容并处理失败。

容器也能标示元素类型。以下写法需要 Python 3.9 或更新版本：

```python
# 输出：
# ['笔', '书']
# 说明：list[str] 表示预期为字符串列表；-> list[str] 表示返回值的预期类型。
...
`strip()` 去除字符串两端空白；本例过滤空字符串或只有空白的名称。`list[str]` 是描述，不是创建列表的操作。只做显示、没有需要返回的结果时，可以把返回提示写成 `-> None`；这里的 `None` 也不等于「没有任何行为」。

类型提示不要和默认值混淆：`quantity: int = 1` 里，`int` 是预期类型，`1` 才是省略实参时使用的值。函数的输入范围、金额单位、是否允许负数，也不是单写 `int` 就能完整说明，仍需文件和检查。
...
第三次交出 `4` 的当下，函数仍暂停在 `yield`。下一次要求取值，才继续走到结尾。`for` 会自动处理最后的耗尽信号。
...
| `factorial(0)` 到基例 | 直接返回 `1` |
| 返回 `factorial(1)` | `1 * 1 = 1` |
| 返回 `factorial(2)` | `2 * 1 = 2` |
| 返回 `factorial(3)` | `3 * 2 = 6` |

对全排列 `permutations(["A", "B"])`：先选 `A` → 剩下 `["B"]` → 再选 `B` → 剩下 `[]` → 基例返回 `[[]]` → 组成 `["B"]` → 组成 `["A", "B"]`。之后回到第一层改选 `B`，得到 `["B", "A"]`。
...
找 `9` 时会经过中间值 `12 → 3 → 7`，最后 `left = 2`、`right = 1`。搜索范围为空，才返回 `-1`。
...
| 「`[x if 条件 for x in ...]` 可以只保留符合值」 | 末尾筛选写 `[x for x in ... if 条件]`；前面选值必须有 `else`。 |
| 「递归有基例就一定会停」 | 每次调用还必须朝基例前进，并符合输入前提。 |
| 「全排列的空输入应返回空列表」 | 组合用的基例需要一个空排列，故用 `[[]]`。 |
| 「二分搜索适用任何列表」 | 必须符合排序前提与一致的比较规则。 |
| 「找到 `-1` 就印 `numbers[-1]`」 | `-1` 是本函数自定义的未找到信号，先判断，再取值。 |
...
| `send(5)` 的返回值是 `5` 吗？ | 不一定。它送入 `5`，然后返回生成器接下来 `yield` 的值。 |
| 写递归前先确认什么？ | 输入范围、基例，以及每一步确实让问题变小。 |
